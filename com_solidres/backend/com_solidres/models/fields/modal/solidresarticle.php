 ...
 * @since  1.6
 */
class JFormFieldModal_Solidresarticle extends JFormField
{
	/**
	 * The form field type.
	 *
	 ...
	 * @since   1.6
	 */
	protected $type = 'Modal_Solidresarticle';
	...
	protected function getInput()
	{
		...
		if ($allowClear && !$scriptClear)
		{
			...
			$script[] = '		document.getElementById(id + "_name").value = "' .
			            htmlspecialchars(JText::_('com_solidres_SELECT_AN_ARTICLE', true), ENT_COMPAT, 'UTF-8') . '";';
		...
		$linkArticles = 'index.php?option=com_solidres&amp;view=articles&amp;layout=modal&amp;tmpl=component'
		                . '&amp;function=jSelectArticle_' . $this->id;

		$linkArticle  = 'index.php?option=com_solidres&amp;view=article&amp;layout=modal&amp;tmpl=component'
		                . '&amp;task=article.edit'
		...
		if (isset($this->element['language']))
		{
			...
			$modalTitle    = JText::_('COM_SOLIDRES_CHANGE_ARTICLE') . ' &#8212; ' . $this->element['label'];
		}
		else
		{
			$modalTitle    = JText::_('COM_SOLIDRES_CHANGE_ARTICLE');
		}
		...
		if ($value)
		{
			...
			if ($app->isSite())
			{
				$query->where($db->quoteName('created_by') . ' = ' . $user->get('id'));
			}
		...
		if (empty($title))
		{
			$title = JText::_('COM_SOLIDRES_SELECT_AN_ARTICLE');
		}
		...
		$html[] = '<a'
		          ...
		          . ' title="' . JHtml::tooltipText('COM_SOLIDRES_CHANGE_ARTICLE') . '">'
		...
		if ($allowEdit)
		{
			$html[] = '<a'
			          ...
			          . ' title="' . JHtml::tooltipText('COM_SOLIDRES_EDIT_ARTICLE') . '">'
		...
		$html[] = JHtml::_(
			...
			array(
				'title'       => JText::_('COM_SOLIDRES_EDIT_ARTICLE'),
				'backdrop'    => 'static',
				'keyboard'    => false,
				'closeButton' => false,
				'url'         => $urlEdit,
